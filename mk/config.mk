...
CPPFLAGS ?= -D_POSIX_C_SOURCE=200809L -Iinclude -Isrc/client -Isrc/core -Isrc/engine -Isrc/protocol -Isrc/query -Isrc/server \
    -Isrc/storage -Isrc/runtime -Isrc/simd
CFLAGS_OPT ?= -O3 -flto
...
WAL_SRC = $(SRC_DIR)/storage/geo_wal.c
...
WAL_OBJ_DEBUG = $(BUILD_DIR)/geo_wal_debug.o
...
WAL_OBJ_SCALAR = $(BUILD_DIR)/geo_wal_scalar.o
...
DEBUG_LIBRARY_OBJECTS = $(LIB_OBJ_DEBUG) $(DENSITY_OBJ_DEBUG) $(STREAM_OBJ_DEBUG) $(PARALLEL_OBJ_DEBUG) \
	$(BATCH_OBJ_DEBUG) $(SEGMENTS_OBJ_DEBUG) $(IO_OBJ_DEBUG) $(THREAD_POOL_OBJ_DEBUG) $(WAL_OBJ_DEBUG) \
	$(DATABASE_OBJ_DEBUG) $(PROTOCOL_OBJ_DEBUG) $(JOB_QUEUE_OBJ_DEBUG) $(CLIENT_OBJ_DEBUG) $(SERVER_OBJ_DEBUG) \
...
SCALAR_LIBRARY_OBJECTS = $(LIB_OBJ_SCALAR) $(DENSITY_OBJ_SCALAR) $(STREAM_OBJ_SCALAR) $(PARALLEL_OBJ_SCALAR) \
	$(BATCH_OBJ_SCALAR) $(SEGMENTS_OBJ_SCALAR) $(IO_OBJ_SCALAR) $(THREAD_POOL_OBJ_SCALAR) $(WAL_OBJ_SCALAR) \
	$(DATABASE_OBJ_SCALAR) $(PROTOCOL_OBJ_SCALAR) $(JOB_QUEUE_OBJ_SCALAR) $(CLIENT_OBJ_SCALAR) $(SERVER_OBJ_SCALAR) \
...
DIRECT_BUILD_TARGETS = $(LIB_OBJ) $(DENSITY_OBJ) $(STREAM_OBJ) $(PARALLEL_OBJ) $(BATCH_OBJ) $(SEGMENTS_OBJ) $(IO_OBJ) \
	$(THREAD_POOL_OBJ) $(WAL_OBJ) $(DATABASE_OBJ) $(PROTOCOL_OBJ) $(JOB_QUEUE_OBJ) $(CLIENT_OBJ) $(SERVER_OBJ) $(SERVER_COMMANDS_OBJ) \
	$(SIMD_OBJ) \
	$(LIB_OBJ_DEBUG) $(DENSITY_OBJ_DEBUG) $(STREAM_OBJ_DEBUG) $(PARALLEL_OBJ_DEBUG) $(BATCH_OBJ_DEBUG) $(SEGMENTS_OBJ_DEBUG) \
	$(IO_OBJ_DEBUG) $(THREAD_POOL_OBJ_DEBUG) $(WAL_OBJ_DEBUG) $(DATABASE_OBJ_DEBUG) $(PROTOCOL_OBJ_DEBUG) $(JOB_QUEUE_OBJ_DEBUG) \
	$(CLIENT_OBJ_DEBUG) $(SERVER_OBJ_DEBUG) $(SERVER_COMMANDS_OBJ_DEBUG) $(SIMD_OBJ_DEBUG) $(LIB_OBJ_SCALAR) \
	$(DENSITY_OBJ_SCALAR) \
	$(STREAM_OBJ_SCALAR) \
	$(PARALLEL_OBJ_SCALAR) $(BATCH_OBJ_SCALAR) $(SEGMENTS_OBJ_SCALAR) $(IO_OBJ_SCALAR) $(THREAD_POOL_OBJ_SCALAR) $(WAL_OBJ_SCALAR) \
	$(DATABASE_OBJ_SCALAR) $(PROTOCOL_OBJ_SCALAR) $(JOB_QUEUE_OBJ_SCALAR) $(CLIENT_OBJ_SCALAR) $(SERVER_OBJ_SCALAR) \
	$(SERVER_COMMANDS_OBJ_SCALAR) \
	$(SCALAR_BACKEND_OBJ) $(LIB_STATIC) $(TEST_BIN) $(DATABASE_TEST_BIN) $(SERVER_TEST_BIN) $(DAEMON_TEST_BIN) \
	$(TEST_BIN_DEBUG) $(DATABASE_TEST_BIN_DEBUG) $(SERVER_TEST_BIN_DEBUG) $(TEST_BIN_SCALAR) $(DATABASE_TEST_BIN_SCALAR) \
	$(SERVER_TEST_BIN_SCALAR) $(DAEMON_BIN) $(DAEMON_BIN_DEBUG) $(DAEMON_TEST_BIN_DEBUG) $(SAMPLE_BIN) $(CLIENT_SAMPLE_BIN) $(DEMO_BIN) \
	$(TOMBSTONE_BENCH_BIN) $(STREAM_BENCH_BIN) $(SERVER_BENCH_BIN) \
	$(COMPACTION_BENCH_BIN) $(SOAK_BIN)

.PHONY: all clean test test-debug test-scalar lib sample client-sample demo benchmark benchmark-tombstones benchmark-stream \
